<?php
	...
	if(isset($_POST['add-car-submit'])){
		...
		$_SESSION['message'] = 'Đã thêm xe thành công';
	...
	if(isset($_POST['update-car-submit'])){
		$file = $_FILES['image']; // Lấy file ảnh mới
		$car->update_car($_POST, $file); // Truyền cả $post và $file
		header("Location: admin.php");
	}
	...
	if(isset($_POST['update-story-submit'])){
		$file = $_FILES['story-image']; // Lấy file ảnh mới
		$story->update_story($_POST, $file); // Gọi hàm update_story (chúng ta sẽ viết hàm này)
		header("Location: admin.php");
	}
	...
	if(isset($_POST['update-product-submit'])){
		$file = $_FILES['image']; // Lấy file ảnh mới
		$product->update_product($_POST, $file); // Truyền cả $post và $file
		header("Location: admin.php");
	}
	...
	if(isset($_POST['update-thought-submit'])){
		$file = $_FILES['image']; // Lấy file ảnh mới
		$thought->update_thought($_POST, $file); // Gọi hàm update_thought (chúng ta sẽ viết hàm này)
		header("Location: admin.php");
	}
	...
	// ====== CODE MỚI THÊM VÀO ======
	// Lấy dữ liệu 7 tấm ảnh để hiển thị
	$gallaries = $gallary->get_gallary();
	$gallery_row = $gallaries->fetch_assoc();
	// ===============================
