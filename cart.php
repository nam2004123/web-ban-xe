...
<html lang="en" dir="ltr">
	<head>
		...
		<title>NC CARS | Giỏ hàng</title>
	</head>
	<body>
		<div class="all">
			<div class="nav">
				<a href="index.php"><i class="fas fa-angle-double-left"></i></a><h1>NC CARS</h1>
			</div>
			<div class="container">
				<div class="results">
					<h3>Giỏ hàng của tôi <span class="cost"><i class="fas fa-donate"></i> <?php echo number_format($total); ?> VNĐ</span> <a href="checkout.php?user_id=<?php echo $user_id; ?>" class="checkout"> Thanh toán </a></h3>
					<div class="product">
                        <?php
                        if($cartss->num_rows == 0){
                            echo '<img class="empty-cart" src="assets/Display_images/empty_cart.png">';
                            echo '<h1 class="empty-h1"> Giỏ hàng trống </h1>';
                        }
                            ...
                            if($row['product_id'] != -1){
                                echo '<div class="item">
                                    ...
                                    <div class="item-info">
                                        ...
                                        <p>'.number_format($row['product_price']).' VNĐ</p>
                                        ...
                                        <form class="" action="product.php?id='.$row['product_id'].'" method="post">
                                            <button type="submit" class="order-btn"> Xem lại</button>
                                        </form>
                                        <form action="cart.php?cnl_id='.$row['id'].'" method="post">
                                            <button class="cnl-btn" type="submit" name="cnl-submit">Xóa</button>
                                        </form>
                                    </div>
                                </div>';
                            }
